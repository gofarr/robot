...
//----------------------------------------------------------------------
//--------------------new thread function-------------------------------
void *commusend(void *client_socket)
{
	int length_sent= 0;
	for(;;)
	{
		...
		pthread_cond_wait(&mylock_ready,&mylock);
		
		
		

		length_sent = send((int)client_socket, img_len, 5, 0);
		//printf("length_sent=%d",length_sent);
		length_sent = send((int)client_socket, imgbuffer, img_length, 0);
		if (length_sent != img_length)
		{
			printf("Server Send Image Failed!length_sent=%d\n",length_sent);
		}
		else
			printf("Server Send Image Success:%d\n",length_sent);
...
int open_camer_device()
{
	...
	if((fd = open("/dev/video4",O_RDWR | O_NONBLOCK)) < 0)
	{
		perror("Fail to open");
		exit(EXIT_FAILURE);
	} 
...
int process_image(unsigned char *addr,int length)
{
	...
	for(y=0;y<480;y=y+2)
	{
		for(x=0;x<640;x=x+2)
		{
			...
			if(( tr>tb+55) && (tr>tg+55) && (tb < 100)&& (tg<100))//红>蓝+55    红>绿+55     b<100   g<100
			{
...
//     其中 vx， vy 是正规化之后的斜率向量（单位向量）。 x0,y0 是直线经过的点。
	...
	if(cflag==4)
	{
		pthread_mutex_trylock(&mylock);
		while(flag)
		{
			imgbuffer[i]=(addr)[i];
			if(i)
			{
				if((addr)[i-1]==0xff && (addr)[i]==0xd9)
					flag=0;
			}
			i++;	
		}
		...
		img_length=i+1;
		sprintf(img_len,"%d",img_length);
		printf("img_len=%s\n",img_len);
		cflag=0;
		pthread_cond_signal(&mylock_ready);
		pthread_mutex_unlock(&mylock);
...
int mainloop(int fd,int client_socket)
{ 
	int count = 0;
	int nsend;
        //int flag = 1;
...
int main()
{
	...
	err=pthread_create(&tfd,NULL,commusend,(void *)client_socket);//确定调用该线程函数的入口点
	if(err!= 0)
	{
